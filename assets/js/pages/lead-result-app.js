...
function renderProfitable(lead) {
  const r = lead.result;
  const savingVal   = rVal(r.annualSavingEur);
  const savingEur   = Math.round(savingVal.max || 0);   // hero shows best (max) saving
  const recFull     = rVal(r.recoveryPctFull);
  const recPartial  = rVal(r.recoveryPctPartial);
  // For energy bar / chart we use the average of min+max self-consumption %
  const selfPctMax  = Math.max(0, Math.min(100, (recFull.max || 0) + (recPartial.max || 0)));
  const selfPct     = selfPctMax;

  // Compute annualized kWh for the 3-segment energy bar.
  let energyBar = null;
  if (lead.csvDailyCompact && Array.isArray(lead.csvDailyCompact.afname)) {
    ...
    if (total > 0) {
      energyBar = {
        ...
        injKwh, batteryKwh, gridKwh,
      };
    }
  }

  return `
    ${heroCard(savingEur, r)}
    ${solutionCard(r)}
    ${energyFlowCard(energyBar)}
    ${usageChartCard(lead, energyBar)}
...
function renderNotProfitable(lead) {
  return `
...
function heroCard(savingEur, r) {
  const savingTxt = fmtEurRange(r.annualSavingEur);
  return `
    <div class="sp-result-card sp-hero-card text-center">
      <div class="sp-hero-eyebrow">Op basis van jouw eigen verbruik</div>
      <div class="sp-hero-amount">${savingTxt}<span class="sp-hero-amount-unit"> per jaar</span></div>
...
function usageChartCard(lead, energyBar) {
  ...
  return `
    <div class="sp-result-card" id="usageChartCard">
      ...
      <p class="sp-chart-caption">Het <span style="color:#16a34a;font-weight:600;">groen</span> is jouw zonne-energie naar het net. Het <span style="color:#0ea5e9;font-weight:600;">blauw</span> is wat de batterij daarvan opvangt. Het <span style="color:#dc2626;font-weight:600;">rood</span> is jouw resterende netverbruik.</p>
    </div>
  `;
}
...
/** Read a result field that may be a plain number (legacy) or { min, max } (range). */
function rVal(field) {
  if (field && typeof field === 'object' && 'min' in field) return field;
  return { min: field, max: field };
}

/** Format a range field: "x" when min===max, "x tot y" otherwise. */

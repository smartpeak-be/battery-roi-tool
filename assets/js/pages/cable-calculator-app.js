...
function safetyLine(input, calculation = null) {
  const thermal = input.thermalAmpacityA == null
    ? 'Thermische limiet niet gecontroleerd.'
    : `Iz ${fmt(input.thermalAmpacityA, 1)} A: ${calculation && calculation.currentA > input.thermalAmpacityA ? 'overschreden' : 'ok'}.`;
  const c1011 = calculation && isProduction(input)
    ...
    : '';
  return `<p class="cable-result-note">${thermal} ${c1011}</p>`;
}
...
function renderCheck(input) {
  ...
  return `${answer(
    ok ? 'Kabel is OK op spanningsval' : 'Spanningsval is te hoog',
...
function renderCapacity(input) {
  ...
  const cards = values.map(value => ({
    ...
    value: `${fmt(value.maxActivePowerKW, 1)} kW`,
    detail: `${fmt(value.voltageDropLimitA, 1)} A`,
    active: value.voltageDropTargetPercent === target,
  }));
  return `${answer(
    `${fmt(chosen.maxActivePowerKW, 1)} kW`,
    `${section(input.sectionMm2)} · ${fmt(chosen.voltageDropLimitA, 1)} A · bij ${fmt(target, 1)}% spanningsval`,
  )}${limitCards(cards)}<p class="cable-result-note">Maximum op spanningsval, niet automatisch thermisch veilig.</p>`;
}
...
function renderDistance(input) {
  ...
  )}${limitCards(cards)}${safetyLine(input)}`;
}

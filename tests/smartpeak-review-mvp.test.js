...
describe('SmartPeak review MVP', () => {
  it('adds a public review page with editable display name and Google copy flow', () => {
    ...
    expect(js).toContain('reviewPhotosPublic');
    expect(js).toContain('SmartPeak mag deze foto’s ook tonen op de website bij mijn review.');
    expect(js).toContain('window.uploadReviewPhotoWithThumb(requestId');
    expect(js).toContain('Gekozen oplossing:');
    expect(js).toContain('solutionSummary: reviewRequest.solutionSummary || null');
    expect(js).not.toContain("value=\"${value}\" ${value === 5 ? 'checked' : ''}");
  ...
  it('adds dashboard review link generation and an embeddable carousel widget', () => {
    ...
    expect(widget).toContain('sp-review-widget__photo');
    expect(widget).toContain('reviewPhotosPublic');
    ...
    expect(rules).toContain("'reviewPhotosPublic'");
    expect(rules).toContain("data.tag == 'review'");
    expect(rules).toContain('data.showOnWebsite is bool');
